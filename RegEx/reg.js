...
regUtil={
	...
	//	if(regUtil.isNull(exp)){
	//		alert("校验表达式不能为空！");
	//		return false;
	//	}else if(regUtil.isNull(str)){
	//		alert("被校验字符串不能为空！");
	//		return false;
	//	}
	//	var regUtil=eval(exp);
	//	return regUtil.test(str);
	//},
	 ...
	 * @returns
	 */
	isNaturalNumber:function(str,strict){
		if(strict){
			var exp="/^([1-9][0-9]*)$/";
			return regUtil.regEnter(exp,str);
		}
		else
			var exp="/^[0-9]*$/";
		return regUtil.regEnter(exp,str);
	...
	n_mNumInt:function(str,n,m){
		if(regUtil.isNull(n)||regUtil.isNull(m)){
			alert("需要校验的数字位数不能为空！");
			return false;
		}
		var exp="/^\d{"+n+","+m+"}$/";
